...
// lib/iot/grafana/dashboard/dashboard.handler.ts
var dashboard_handler_exports = {};
__export(dashboard_handler_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(dashboard_handler_exports);
...
var handler = async (event) => {
  ...
  switch (RequestType) {
    case "Create": {
      const { data } = await axios_default.post(url2 + "/api/dashboards/db", payload, configs);
      return { PhysicalResourceId: data.uid, Data: { url: data.url } };
    }
    case "Update": {
      const { PhysicalResourceId } = event;
      await axios_default.delete(url2 + "/api/dashboards/uid/" + PhysicalResourceId, configs);
      const { data } = await axios_default.post(url2 + "/api/dashboards/db", payload, configs);
      return { PhysicalResourceId: data.uid, Data: { url: data.url } };
    }
    case "Delete": {
      const { PhysicalResourceId } = event;
      axios_default.delete(url2 + "/api/dashboards/uid/" + PhysicalResourceId, configs).catch(() => {
      });
      return {};
    }
    default:
