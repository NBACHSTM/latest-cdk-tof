...
// lib/iot/grafana/datasource/datasource.handler.ts
var datasource_handler_exports = {};
__export(datasource_handler_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(datasource_handler_exports);
...
var handler = async (event) => {
  ...
  const { SecretString: apiKey } = await secretsmanager.getSecretValue({ SecretId: apiKeySecretName }).promise();
  const url2 = "https://" + endpoint;
  const configs = { headers: { Authorization: "Bearer " + apiKey } };
  const payload = JSON.parse(ResourceProperties.payload);
  switch (RequestType) {
    case "Create": {
      const { data } = await axios_default.post(url2 + "/api/datasources", payload, configs);
      const id = data.datasource.id.toString();
      return { PhysicalResourceId: id, Data: { id } };
    }
    case "Update":
      const { PhysicalResourceId } = event;
      await axios_default.put(url2 + "/api/datasources/" + PhysicalResourceId, payload, configs);
      return { Data: { id: PhysicalResourceId } };
    case "Delete": {
      const { PhysicalResourceId: PhysicalResourceId2 } = event;
      axios_default.delete(url2 + "/api/datasources/" + PhysicalResourceId2, configs).catch(() => {
      });
      return {};
    }
    default:
