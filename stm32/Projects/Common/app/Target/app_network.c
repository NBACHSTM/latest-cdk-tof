...
extern const char *classes_table[8];
extern const int evk_label_table[8];
extern AppConfig_TypeDef App_Config;
/* Private macro -------------------------------------------------------------*/
#define RANGING_OK_5                              (5U)
#define RANGING_OK_9                              (9U)

#define THRESHOLD_NN_OUTPUT                       (0.88)
...
static int ValidateFrame(HANDPOSTURE_Data_t *AI_Data, HANDPOSTURE_Input_Data_t *Input_AI_Data)
{
  ...
  {
    if ((Input_AI_Data->nb_targets[idx] > 0) && (Input_AI_Data->target_status[idx] == RANGING_OK_5 || Input_AI_Data->target_status[idx] == RANGING_OK_9) && Input_AI_Data->ranging[idx] < min)
    {
  ...
  valid = (Input_AI_Data->nb_targets[idx] > 0)
        && (Input_AI_Data->target_status[idx] == RANGING_OK_5 || Input_AI_Data->target_status[27] == RANGING_OK_9)
...
void Network_Inference(AppConfig_TypeDef *App_Config)
{
 ...
 // }
  /*else
  {LogError( " \033[1;31m No frame detected  AI model waiting for input.. \033[0m \n ");
